...
replace "s|@@RELEASE_DOWNLOAD_REF@@|${RELEASE_DOWNLOAD_REF}|g" release_notes.tmp.md # zhanlu_change - use the draft's untagged URL when needed
...
gh release edit "${VERSION}" --repo "${ASSETS_REPOSITORY}" --notes-file release_notes.tmp.md "${DRAFT_FLAG}"
sync_release_download_ref "${ASSETS_REPOSITORY}" "${VERSION}" "${DRAFT_FLAG}"
# zhanlu_change end
