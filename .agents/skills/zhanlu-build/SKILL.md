...
- Store this skill in the current `/Volumes/Files/vscodium` project, but run
  ...
  this skill.
- Use `develop` for zhanlu-code. Use the immutable `upstream/stable.json` commit
  for zhanlu-core until the native Agent migration is present on `develop`.
...
- For an interactive wrapper invocation, preview and then synchronize the four
...
- For build-portal invocations, preview an immutable plan containing `develop`
  for all four components plus any distinct selected zhanlu-code and
  zhanlu-core refs. Apply that exact plan with
...
- Keep the GitHub Release as draft unless the user explicitly requests
  publication.
- Sync the four component GitLab repositories for formal releases; do not sync
...
4. Ask the user to confirm that exact plan. Explain that applying it may push a
   ...
   for four components, and dispatch workflows.
...
6. During an interactive apply, run
   ...
   for all four `develop` refs plus distinct selected build refs. Apply only
...
Use `--source-branch`, `--zhanlu-core-ref`, `--delivery-profile`, or
`--time-patch` only when the user explicitly selects a
...
- For direct interactive builds, use the synchronization script's
  default-branch mode when both build refs are `develop`; add `--all-refs`
...
- For portal builds, GitLab is authoritative. Before the first remote write,
  preview all four component `develop` refs plus distinct selected build refs.
...
- Before formal GitLab synchronization, require `zhanlu-cloud`, `zhanlu-code`,
  `zhanlu-core`, and `zhanlu-loc` to be clean, on `develop`, and
