...
printf '%s' "$repositories_json" | jq -e 'type == "object" and (has("vscodium") and has("zhanlu-cloud") and has("zhanlu-code") and has("zhanlu-core") and has("zhanlu-loc"))' >/dev/null || { echo "REPOSITORIES_JSON must include vscodium and the four native-Agent components" >&2; exit 2; } # zhanlu_change
