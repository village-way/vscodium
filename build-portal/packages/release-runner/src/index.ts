...
export async function runRelease(spec: BuildSpec, requestId: string, options: { workspace: string; apply: boolean; confirmedSourcePlan: string; sourceRefs?: SourceRefs; onLog?: (line: string) => Promise<void> }): Promise<RunnerResult> {
  ...
  const consumeLine=async(raw:string)=>{if(!raw)return;const line=redact(raw);if(line.startsWith("{"))jsonLines.push(line);const candidate=line.replace(ANSI_PATTERN, "").trim();const priority=/^remote:\s*(?:error:|fatal:|refusing\b)/i.test(candidate)?4:/^\[ERROR\]/i.test(candidate)?3:/^(?:fatal:|error:)/i.test(candidate)&&!/^ERROR:\s*command failed\b/i.test(candidate)?2:/^ERROR:/i.test(candidate)?1:0;if(priority>=failurePriority&&priority>0){failureHint=candidate;failurePriority=priority;}await options.onLog?.(line);};
