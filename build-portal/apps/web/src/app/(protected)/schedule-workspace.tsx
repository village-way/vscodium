...
type Draft = { name: string; cron: string; version: string; platform: string; outputMode: string };
const emptyDraft: Draft = { name: "", cron: "5 1 * * *", version: "", platform: "linux", outputMode: "workflow-artifact" };
...
export function ScheduleWorkspace() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try { const url = editing ? `/api/schedules/${editing}` : "/api/schedules"; const method = editing ? "PATCH" : "POST"; await apiJson(url, { method, headers: writeHeaders(csrfToken), body: JSON.stringify({ name: draft.name, cron: draft.cron, timezone: "Asia/Tokyo", enabled: true, spec: specFor(draft) }) }); setMessage(editing ? "规则已更新" : "规则已创建"); setDraft(emptyDraft); setEditing(null); await refresh(); }
  ...
  function edit(schedule: Schedule) { setEditing(schedule.id); setDraft({ name: schedule.name, cron: schedule.cron, version: schedule.spec.version, platform: schedule.spec.platform, outputMode: schedule.spec.outputMode }); window.scrollTo({ top: 0, behavior: "smooth" }); }
