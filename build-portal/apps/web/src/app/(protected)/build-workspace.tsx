...
export function BuildWorkspace() {
  ...
  const uniqueRefs = (items: RefSnapshot[]) => Array.from(new Map(items.map((item) => [item.ref, item])).values());
  const groupedRefs = useMemo(() => ["zhanlu-code", "zhanlu-core", "zhanlu-vs"].map((repository) => ({ repository, items: refs.filter((item) => item.repository === repository) })), [refs]);
