...
async function handler(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  if (id && path.length === 2 && method === "GET") { const build = await getPool().query("SELECT b.*,u.username AS requested_by_username FROM builds b LEFT JOIN users u ON u.id=b.requested_by WHERE b.id=$1", [id]); if (!build.rows[0]) throw new HttpError("build not found", 404); const runs = await getPool().query("SELECT platform,workflow,run_id,run_url,status,conclusion,dispatch_attempts,created_at,updated_at FROM build_runs WHERE build_id=$1 ORDER BY platform", [id]); return json({ build: build.rows[0], runs: runs.rows }); }
